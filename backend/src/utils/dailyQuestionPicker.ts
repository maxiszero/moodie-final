...
/** @typedef {'light'|'heavy'|'neutral'} MoodBucket */

/** Same calendar day UTC for everyone */
 ...
 * @returns {MoodBucket}
 */
function getMoodBucket(rawEmotion) {
  const e = normalizeEmotion(rawEmotion);
  const light = new Set(['happy', 'excited', 'loved', 'inspiration', 'calmness', 'drive']);
  const heavy = new Set([
    'sad',
    'angry',
    'scared',
  ...
  if (light.has(e)) return 'light';
  if (heavy.has(e)) return 'heavy';
  return 'neutral';
}
 ...
 * @param {MoodBucket} bucket
 * @param {'ru'|'en'} lang
 */
function pickQuestion(dayKey, bucket, lang) {
  const L = lang === 'en' ? 'en' : 'ru';
  const list = POOLS[L][bucket] || POOLS[L].neutral;
  const idx = djb2(`${dayKey}:${bucket}:${L}`) % list.length;
  return list[idx];
}
